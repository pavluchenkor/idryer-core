...
ledcWrite(PWM_CHANNEL, 0);  // desligado na inicialização
```
...
!!! warning
    `ledcSetup` / `ledcAttachPin` é a API Arduino ESP32 para versões anteriores a 3.x. Na versão 3.x e posteriores use `ledcAttach(pin, freq, resolution)` e `ledcWrite(pin, duty)`. Verifique sua versão em `platformio.ini` (`platform = espressif32@X.Y.Z`).
