...
**Storage Link não utiliza o módulo UART** — é um dispositivo ESP32-C3 independente sem um segundo MCU.
...
- UART 8N1, 115200 baud (predefinido), sem controlo de fluxo de hardware.
...
Classe principal do módulo. Processa o stream de entrada byte a byte, constrói quadros, verifica CRC, gerencia ACK/retry e envia quadros para callbacks registados.

```cpp
UartBridge bridge;
bridge.begin(&Serial1, 115200);

// Registar handlers antes de begin()
...
Dispositivos dual-MCU requerem o número de série de RP2040 antes do aprovisionamento:
