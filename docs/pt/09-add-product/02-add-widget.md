...
> O conceito de widget foi redefinido: agora um widget é um **cartão de dispositivo no dashboard do portal** (componente React específico do produto), não um artefacto gerado a partir de `contracts/widgets/`. A camada antiga `widget-registry`/`contracts/widgets/` foi removida.
