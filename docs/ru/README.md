---
title: "idryer-core для устройств iDryer и самодельных модулей"
description: "Документация idryer-core: как подключить ESP32-устройство, MQTT, Wi-Fi, облачную стейт-машину и команды для сушилки филамента или модуля 3D-принтера."
---

# idryer-core для устройств iDryer и самодельных модулей

`idryer-core` нужен, когда самодельная сушилка филамента, термокамера, подсветка или другой модуль 3D-принтера должен стать управляемым устройством iDryer. Библиотека берёт на себя Wi-Fi, MQTT, команды, телеметрию и связь с порталом, а продуктовый код описывает только поведение конкретного устройства.
