---
title: "Что такое idryer-core и когда он нужен"
description: "Назначение idryer-core: общая библиотека для ESP32-устройств iDryer, MQTT, Wi-Fi, телеметрии и команд от портала."
---

# Что такое idryer-core и когда он нужен

`idryer-core` стоит использовать, если устройство должно подключаться к экосистеме iDryer: публиковать телеметрию, принимать команды, проходить привязку и работать через MQTT. Это особенно полезно для сушилок филамента, активных камер, подсветки, датчиков и других модулей вокруг 3D-принтера.
