> **⚠ Veraltete Übersetzung (Stand 2026-05-27).**
> Das Widget-Konzept wurde neu definiert: Ein Widget ist jetzt eine **Gerätekarte im Portal-Dashboard** (produktspezifische React-Komponente), kein generiertes Artefakt aus `contracts/widgets/`. Die alte Schicht `widget-registry`/`contracts/widgets/` wurde entfernt.
> Die englische Version unter [../en/09-add-product/02-add-widget.md](../../en/09-add-product/02-add-widget.md) ist die aktuelle Quelle der Wahrheit. Diese Übersetzung wird separat aktualisiert.
