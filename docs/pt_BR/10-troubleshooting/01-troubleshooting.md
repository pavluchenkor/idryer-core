...
Antes de ler, certifique-se de que os registros HAL estão ativados (`idryer::hal::initArduinoHal(&Serial)`) e que `-DCORE_DEBUG_LEVEL=3` ou superior está definido em `platformio.ini`.

## WiFi

### Máquina de estado presa em `WifiConnecting`

Sintomas: o registro repete `state: WifiConnecting`, a transição para `Provisioning` nunca acontece.

Possíveis causas:

- SSID/senha incorreta. Verifique `WIFI_SSID` / `WIFI_PASSWORD` em `secrets.h`. Após provisioning Improv, as credenciais vêm do NVS, não de `secrets.h`.
- Rede 5 GHz. ESP32 suporta apenas 2,4 GHz.
- Rede oculta ou filtro MAC no roteador.
- `WiFi.begin()` chamado antes de `idryer::hal::initArduinoHal(...)` — sem saída de registro, mas isto não é a causa da suspensão, apenas cegueira.
...
Registrar RSSI no loop do produto:
...
Este é o estado normal enquanto o usuário não tiver entrado no PIN no portal. O PIN é impresso no registro via `setClaimPinCallback`.

Para reclamação automática (dispositivos autônomos sem UI):

```cpp
s_cloud.setUnclaimedCallback([](void*) { s_cloud.requestClaim(); }, nullptr);
```

Após `requestClaim()`, o backend emite um PIN que o usuário deve entrar no portal.
...
Verifique a subscrição — `MqttClient` subscreve a `idryer/{serial}/commands/#` com QoS 1. Se a subscrição falhar, o registro mostrará:
...
Se registrou `setCommandHandler()`, **o fallback incorporado para `ActionDispatcher` está desativado**. `IdryerRuntime` passa tudo (exceto `ping`) para seu `CommandHandler`. Você deve chamar explicitamente `s_dispatcher.handleInvoke(data)` lá para comandos `invoke`.
...
Deve haver **uma** versão. Se houver duas — pindele-a explicitamente via `lib_deps` e se necessário via `lib_ldf_mode = chain+` ou `lib_ignore`.
...
- O roteador não bloqueia multicast.
...
Útil para diagnóstico remoto: o usuário ativa `DIAG LOG`, copia a saída → estados de conector, lastError, e o que está realmente indo para RMT são visíveis.
...
O prefixo `[!]` destaca visualmente a anomalia no fluxo de registro geral. Esta é a primeira coisa a procurar em Serial quando algo "não está funcionando".
