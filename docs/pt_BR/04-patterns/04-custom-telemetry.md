# Telemetria customizada (carga específica do produto)
...
- **Não publique ambos** a telemetria automática idryer-core (não-zero `telemetryPeriodMs`) e telemetria customizada simultaneamente. O backend recebe duas mensagens no mesmo tópico e processa ambas — dados são duplicados.
