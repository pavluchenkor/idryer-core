...
description: "新しいiDryerデバイスを追加するためのチェックリスト: プロファイル、コマンド、テレメトリ、MQTT、ポータル、ライブラリと製品コードの境界。"
---

# idryer-coreベースの新しい製品を追加する

このガイドは、`idryer-core` の上に新しい製品を作るときに使います。対象はフィラメント乾燥機、加熱ブロック、照明、センサー、その他のモジュールです。ライブラリに残す部分と、個別製品コードに書く部分を示します。
