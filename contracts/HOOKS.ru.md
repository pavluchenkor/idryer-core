...
Этот hook защищает от commit'ов с устаревшим или невалидным `mqtt_contract.yaml` /
...
После клона репозитория один раз (запускается из корня репозитория):
...
### ❌ Generated файл устарел (забыл регенерировать)
```
$ git commit -m "..."
...
Запуск напрямую:
```bash
python3 contracts/validate_contract.py        # проверить yaml
python3 contracts/gen_uart_protocol_h.py      # перегенерировать .h
```
