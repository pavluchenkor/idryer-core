...
// ── Config ────────────────────────────────────────────────────────────────────
// Flags generated from device_profiles.dryer_v3 in mqtt_contract.yaml.
...
class DryerV3Profile : public idryer::IProfile {
public:
...
// ── Product layer ─────────────────────────────────────────────────────────────
static DryerV3Profile s_profile;
...
static void handleCommand(const char *cmd, JsonObjectConst data) {
  ...
  if (strcmp(cmd, "get_config") == 0 || (strcmp(cmd, "invoke") == 0 && strcmp(action, "device.getConfig") == 0)) {
  ...
  if (strcmp(cmd, "invoke") == 0) {
    s_dispatcher.handleInvoke(data);
    return;
  }
  if (strcmp(cmd, "set") == 0) {
    s_dispatcher.handleSet(data);
    return;
  }
...
static bool onInvoke(const char *action, JsonObjectConst args, void * /*ctx*/) {
  // No invoke actions defined for this profile yet.
  (void)action;
  (void)args;
  return false;
}
