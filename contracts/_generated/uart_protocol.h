...
struct UartHelloPayload {
    ...
    char    hardwareVersion[16];  ///< HW-идентификатор контроллера (rp2040-v1, stm32f103-v1)
...
static_assert(sizeof(UartHelloPayload) == 94, "UartHelloPayload must be 94 bytes (yaml-computed)");
...
struct UartOtaChunkForMcuPayload {
    ...
    uint16_t    totalChunks;  ///< копия из MQTT announce для контроля целостности
    uint16_t    dataLength;  ///< фактический размер data[] этого chunk'а (последний может быть короче)
    uint16_t    _pad;  ///< padding до 4 + 2 + 2 + 2 + 2 = 12 байт заголовка
} __attribute__((packed));
static_assert(sizeof(UartOtaChunkForMcuPayload) == 12, "UartOtaChunkForMcuPayload must be 12 bytes (yaml-computed)");

/// Ответ RP на принятый chunk: ok / sha-fail / flash-fail.
struct UartOtaChunkAckPayload {
    uint32_t    commandId;
    uint16_t    chunkIdx;
    uint8_t    status;  ///< 0=ok, 1=sha_mismatch, 2=flash_failed, 3=out_of_order, 4=retry (chunk пришёл непо
    uint8_t    _pad;  ///< padding до 8 байт
} __attribute__((packed));
static_assert(sizeof(UartOtaChunkAckPayload) == 8, "UartOtaChunkAckPayload must be 8 bytes (yaml-computed)");
...
struct UartOtaCommitNowPayload {
    uint8_t    targetMajor;  ///< ожидаемый major после reboot — для верификации в post-handshake
    uint8_t    _pad;  ///< padding до 2 байт (минимум для not-empty payload)
} __attribute__((packed));
static_assert(sizeof(UartOtaCommitNowPayload) == 2, "UartOtaCommitNowPayload must be 2 bytes (yaml-computed)");
...
struct UartOtaStatusPayload {
    uint8_t    espReady;  ///< 0/1 — ESP свою прошивку скачала и валидировала
    uint8_t    espTargetMajor;  ///< major скачанной у ESP прошивки, или 0 если nothing pending
